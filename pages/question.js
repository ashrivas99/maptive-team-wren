...
export default function Question() {
        ...
        return (
          <div className="flex items-center space-x-3">
            <p className="text-red-500 text-lg font-bold">
              You are incorrect! The answer was {answer}
            </p>
            <p className="text-3xl">😢</p>
          </div>
        );
          ...
          <div className="space-y-4">
            {answers.map((answerOption, ind) => (
              <div key={ind} className="flex items-center">
                ...
                />
                {answerTextExists(data[index], answerOption) && (
                  <label
                    htmlFor={ind}
                ...
                {answerImageExists(data[index], answerOption) && (
                  <label
                    htmlFor={ind}
                    className="ml-3 block text-sm font-medium text-gray-700"
                  >
  ...
  return (
    <div className="h-screen flex items-center justify-center">
      <div>
        {textExists(data[index]) && (
          <p className="text-center p-5 text-2xl font-bold text-indigo-700">
            {data[index].mcq.q_text}
          </p>
        )}
        {imageExists(data[index]) && (
          <img
            src={"https://mathopolis.com/questions/" + data[index].mcq.q_image}
          ></img>
        )}
        <Options></Options>
        <Answer></Answer>
